<?php
// owner_list.php - Стилізований список власників
session_start();
...
$search_where = ""; $sort_order_clause = "";
...
if (isset($_GET['search_term']) && $_GET['search_term'] !== '') {
    ...
    if (in_array($field, $allowed_fields)) { $search_where = "WHERE V.{$field} LIKE '%{$term}%'"; }
}
...
$sort_order_clause = "ORDER BY V.{$sort_by} {$sort_order}";

$sql_select = "SELECT * FROM VLASNYK V {$search_where} {$sort_order_clause}";
...
function sort_link($field, $label, $current_by, $current_order) {
    ...
    return "<a href='owner_list.php?{$query_string}' class='sort-link'>{$label}{$icon}</a>";
}
?>
<!DOCTYPE html>
<html lang="uk">
<head>
    <meta charset="UTF-8">
    <title>Власники | Панель управління</title>
...
<body>
    <div class="container">
        ...
        <form method="get" action="owner_list.php" class="filter-panel">
            ...
            <div>
                ...
                <a href="owner_list.php" class="btn btn-secondary">Скинути</a>
            </div>
        </form>

        <?php if ($is_admin): ?>
            <div style="margin-bottom: 20px;">
                <a href="../add_edit_delete/add_owner.php" class="btn btn-success">+ Додати Власника</a>
            </div>
        <?php endif; ?>
        ...
        <div style="overflow-x: auto;">
            <table>
                <thead>
                    <tr>
                        <th width="5%"><?= sort_link('id_vlasnyk', 'ID', $sort_by, $sort_order) ?></th>
                        <th width="35%"><?= sort_link('pib', 'ПІБ/Назва', $sort_by, $sort_order) ?></th>
                        <th width="20%"><?= sort_link('telefon', 'Телефон', $sort_by, $sort_order) ?></th>
                ...
                if (mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_assoc($result)) {
                        echo "<tr>";
                        echo "<td>#" . $row['id_vlasnyk'] . "</td>";
                        echo "<td>" . htmlspecialchars($row['pib']) . "</td>";
                        echo "<td>" . $row['telefon'] . "</td>";
                        echo "<td>" . htmlspecialchars($row['email']) . "</td>";
                        if ($is_admin) {
                            echo "<td>";
                            echo "<a href='../add_edit_delete/edit_owner.php?id=" . $row['id_vlasnyk'] . "' class='btn btn-sm btn-edit'>✎</a>";
                            echo "<a href='../add_edit_delete/data_handler.php?action=delete_owner&id=" . $row['id_vlasnyk'] . "' class='btn btn-sm btn-delete' onclick='return confirm(\"Видалити?\")'>🗑</a>";
                            echo "</td>";
                        }
                        echo "</tr>";
                    }
                } else {
                    echo "<tr><td colspan='" . ($is_admin ? 5 : 4) . "' style='text-align:center; padding:30px; color:#888;'>Немає записів.</td></tr>";
                }
                ?>
                </tbody>
            </table>
